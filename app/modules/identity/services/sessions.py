...
from app.modules.identity.repositories.sessions import (
    get_session_by_refresh_hash,
    mark_session_family_compromised,
...
async def rotate_refresh_session(
    ...
    *,
    ...
    existing = await get_session_by_refresh_hash(db, hash_token(refresh_token), for_update=True)
    ...
    if existing.revoked_at is not None or existing.replaced_by_id is not None:
        await mark_session_family_compromised(db, existing.family_id, now)
        await db.commit()
...
async def get_refresh_session(db: AsyncSession, refresh_token: str) -> AuthSession | None:
    return await get_session_by_refresh_hash(db, hash_token(refresh_token))
...
async def revoke_refresh_session(db: AsyncSession, refresh_token: str) -> None:
    await revoke_session_by_refresh_hash(db, hash_token(refresh_token), datetime.now(UTC))
    await db.commit()


async def revoke_auth_session(db: AsyncSession, session: AuthSession) -> None:

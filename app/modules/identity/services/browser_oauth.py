...
from app.modules.workspaces.repositories import (
    get_active_workspace_with_role,
...
async def provision_oauth_session(
    ...
    *,
    ...
    if identity is None:
        ...
        existing_membership = await get_first_active_membership(db, existing_user.id)
...
async def get_bound_session(
    ...
    workspace_role = await get_active_workspace_with_role(
        db, auth_row.AuthSession.current_workspace_id, user_id
    )
...
async def delete_user_account(db: AsyncSession, user: User) -> None:
    ...
    stored_user.deleted_at = now
    stored_user.email = None
    stored_user.github_id = None
    stored_user.github_username = None
    await db.commit()

...
class DraftRejection(ApiSchema):
    reason: Annotated[str, StringConstraints(strip_whitespace=True, min_length=1, max_length=280)]
...
class DraftAngleRegeneration(ApiSchema):
    angle: Annotated[
        str,
        Field(
            pattern="^(user_benefit|problem_solved|shipping_update|technical_credibility|founder_build_in_public|milestone_or_traction)$"
        ),
    ]

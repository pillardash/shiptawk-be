...
from app.domains.legacy.models import drafts, event_evaluations, repos, tweet_candidates
    ...
    draft = (
        (
            await db.execute(
                select(*draft_response_columns)
                .select_from(
                    draft_repo_join.join(
                        WorkspaceMembership,
                        WorkspaceMembership.workspace_id == drafts.c.workspace_id,
                    )
                )
                .where(
